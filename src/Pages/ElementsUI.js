      ...
      {user === "admin" ? (
        ...
        <>
          ...
          <SearchBar style={{ display: searchBarVisiblity ? 'block' : 'none' }}>
            ...
            <SerachResulte style={{ display: listVisiblity ? 'block' : 'none' }}>
              {query ?
                allContent.map((content) => content.map((result) => <QueryResult onClick={() => navigate(`/${user}/view/${result.tag === 'كتاب' ? 'Books' : result.tag === 'فيديو' ? 'Video' : 'Audio'}/${result.id}`)}><div style={{ position: 'relative', width: '100%', display: 'flex', alignItems: 'center' }}><span>{query}</span><span style={{ position: 'absolute', right: 0, color: "black", opacity: "0.3", zIndex: "-1", boxSizing: "border-box" }}>{result.title}</span></div><span>{result.tag}</span></QueryResult>))
                :
                <span style={{ opacity: ".5" }}>محتوى غير متوفر</span>
              }
            </SerachResulte>
        ...
        <Responsive>
          ...
          >
            ...
            <Description>
              ...
              <p style={{ margin: "0" }}>لكتاب هو مجموعة من الصفحات المكتوبة أو الفارغة المرتبطة ببعضها بغراء
                أو مُحاكة بخيوط من جهة واحدة لتلتصق ببعضها البعض، يحميها من
                الجانبين الخارجيين غلاف قاسٍ، وقد يكون الكتاب عبارة عن مجموعة من الصفحات المكتوبة إلكترونيً</p>
            </Description>
